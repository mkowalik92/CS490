...
async function populateExamResults(examId, studentId) {
  ...
  //console.log(JSON.parse(responseParsed));
  ...
  for (var y = 0; y < response2JSON.length; y++) {
    ...
    replacementText += "Question Id: " + response2JSON[y].questionId + "<div>Correct: " + isCorrect + "</div><div>Function Name Matched: " + functionNameMatched + "</div><div>Points Awarded: <input id='new_point_values[]' value='"
...
async function saveChanges(examId, studentId) {
  var newPointValues = document.querySelectorAll('[id^="new_point_values[]"]');
  const response = await fetch("https://web.njit.edu/~mk343/cs490/rv/examiner/answers_handler.php", {
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded; charset=utf-8'
      },
    credentials: "include",
  ...
  const responseJSON = await response.json();
  var questionCount = 0;
  for (var y = 0; y < responseJSON.length; y++) {
    if (examQuestionIds.indexOf(responseJSON[y].questionId) == -1 || responseJSON[y].studentId != studentId || responseJSON[y].examId != examId) continue;
    await fetch("https://web.njit.edu/~mk343/cs490/rv/examiner/answers_handler.php", {
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded; charset=utf-8'
        },
      credentials: "include",
      body: "action=UPDATEPOINTS&answerId=" + responseJSON[y].answerId + "&points=" + newPointValues[questionCount].value,
      method: "POST"
    });
    questionCount++;
  }
